(function () {
  ...
  const RSI_INTERVALS = [1, 5, 15, 30, 60];
  ...
  window.RSI_load = function () {
    showStatus("Загрузка...", false);

    fetch(`/api/settings/indicator/?name=${INDICATOR_RSI}`)
      .then(r => r.json())
      .then(data => {
        apply(data.json || {});
  ...
  function apply(settings) {
    const root = document.getElementById("indicator-rsi-root");
  ...
  function resetForm(root) {
    ...
    root.querySelectorAll(".field-status-toggle").forEach(t => t.checked = false);
